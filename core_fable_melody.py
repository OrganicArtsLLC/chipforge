"""Core Fable v5 — Classical Film Score.

8-channel orchestral chip tune score for the 140s animated fable.
Key: E natural minor. BPM 116 (stately classical tempo).
Borrows: Moonlight Waltz harp arpeggios, Aqualung flute phrasing,
         Clocks repeating arp engine. Original journey motif E→G→A→B.

Channels:
  0  kick_deep          — heartbeat pulse, sparse in quiet sections
  1  snare_fat + ride   — gentle percussion texture
  2  hat_lo_fi          — soft brushed ticking
  3  bass_sub           — deep sine roots, long sustain
  4  ks_harp            — arpeggiated strings (Moonlight Waltz style)
  5  pad_choir          — choral pad, warmth floor
  6  add_flute          — lyrical lead melody (Aqualung style)
  7  shaped_bell        — accent chimes at key moments

Duration: ~140s (1152 steps at BPM 116, SPB 4).
Acts: starfield(64) → cosmic_grief(96) → gifts_journey(160) →
      labyrinth(128) → thread_of_hope(80) → unraveling(128) →
      stood_up(64) → universe_returns(96) → walk_home(96) →
      fadeout(128) → silence(32) → tail(32)   = 1104 steps

Film timing: 1104 steps × (60/116/4) = 142.8s → covers 140s film.
"""
import sys, os, math
...
# ── Instruments ──────────────────────────────────────────────────
KICK    = "kick_deep"       # ch 0
SNARE   = "snare_fat"       # ch 1 (also ride)
RIDE    = "ride"            # ch 1 alt
HAT     = "hat_lo_fi"       # ch 2
BASS    = "bass_sub"        # ch 3
HARP    = "ks_harp"         # ch 4 — arpeggiated strings
CHOIR   = "pad_choir"       # ch 5 — warm choral pad
FLUTE   = "add_flute"       # ch 6 — lyrical lead
BELL    = "shaped_bell"     # ch 7 — accent chimes
...
C5, D5, E5, Fs5, G5, A5, B5 = 72, 74, 76, 78, 79, 81, 83
E6 = 88

# ── Helpers ──────────────────────────────────────────────────────
def n(inst: str, midi: int, vel: float = 0.70, dur: int = 2) -> NoteEvent:
    return NoteEvent(midi_note=midi, velocity=min(vel, 0.85),
                     duration_steps=dur, instrument=inst)

def grid(steps: int) -> list[list]:
    return [[None] * steps for _ in range(NCH)]

# ── Harp arpeggio helpers (Moonlight Waltz style) ────────────────
def harp_arp_up(g, step, root, third, fifth, octave, vel=0.55, dur=2):
    """Ascending broken chord over 8 steps."""
    notes = [root, third, fifth, octave, fifth, third, root, fifth]
    for i, midi in enumerate(notes):
        if step + i < len(g[4]):
            g[4][step + i] = n(HARP, midi, vel, dur)

def harp_arp_down(g, step, octave, fifth, third, root, vel=0.50, dur=2):
    """Descending broken chord over 8 steps."""
    notes = [octave, fifth, third, root, third, fifth, octave, fifth]
    for i, midi in enumerate(notes):
        if step + i < len(g[4]):
            g[4][step + i] = n(HARP, midi, vel, dur)

# ── Flute melody helpers (Aqualung lyrical style) ────────────────
def flute_phrase(g, step, notes_durs, vel=0.65):
    """Place a flute melodic phrase. notes_durs = [(midi, dur), ...]."""
    pos = step
    for midi, dur in notes_durs:
        if pos < len(g[6]):
            g[6][pos] = n(FLUTE, midi, vel, dur)
        pos += dur

# ── Gentle percussion pattern ────────────────────────────────────
def add_gentle_drums(g, start, bars, vel_k=0.35, vel_h=0.22, vel_r=0.25):
    """Heartbeat kick + soft hat ticking + ride accents."""
    for bar in range(bars):
        base = start + bar * BAR
        # Kick on beat 1 only (heartbeat)
        if base < len(g[0]):
            g[0][base] = n(KICK, 36, vel_k, 2)
        # Hats: soft ticking on every other step
        for s in range(0, BAR, 2):
            pos = base + s
            if pos < len(g[2]):
                g[2][pos] = n(HAT, 42, vel_h, 1)
        # Ride on beat 3 (step 8)
        rpos = base + 8
        if rpos < len(g[1]):
            g[1][rpos] = n(RIDE, 51, vel_r, 2)

def add_full_drums(g, start, bars, vel_k=0.50, vel_s=0.40, vel_h=0.28):
    """Fuller drum pattern for climactic sections."""
    for bar in range(bars):
        base = start + bar * BAR
        # Kick: beats 1, 3
        for beat in [0, 8]:
            pos = base + beat
            if pos < len(g[0]):
                g[0][pos] = n(KICK, 36, vel_k, 2)
        # Snare: beats 2, 4
        for beat in [4, 12]:
            pos = base + beat
            if pos < len(g[1]):
                g[1][pos] = n(SNARE, 38, vel_s, 2)
        # Hats: 8th notes
        for s in range(0, BAR, 2):
            pos = base + s
            if pos < len(g[2]):
...
def p_starfield() -> Pattern:
    """Act I opening — vast cosmic emptiness. Choir + harp only. 64 steps (4 bars)."""
    g = grid(64)
    # Choir pad: E minor drone, very slow
    g[5][0]  = n(CHOIR, E3, 0.45, 16)
    g[5][16] = n(CHOIR, B3, 0.40, 16)
    g[5][32] = n(CHOIR, E3, 0.42, 16)
    g[5][48] = n(CHOIR, G3, 0.38, 16)
    # Harp: lonely ascending arpeggio (like Moonlight intro)
    harp_arp_up(g, 8, E4, G4, B4, E5, vel=0.35, dur=3)
    harp_arp_down(g, 32, E5, B4, G4, E4, vel=0.30, dur=3)
    # Single bell: the first star
    g[7][4]  = n(BELL, E5, 0.30, 4)
    g[7][40] = n(BELL, B4, 0.25, 4)
    return Pattern(name="starfield", bpm=BPM, steps_per_beat=SPB,
                   num_steps=64, num_channels=NCH, grid=g)


def p_cosmic_grief() -> Pattern:
    """Act I — 'Once there was a boy.' Flute enters with the journey motif. 96 steps."""
    g = grid(96)
    # Choir: Em → C → Am → Em
    for i, (note, vel) in enumerate([(E3, 0.50), (C4, 0.48), (A3, 0.45), (E3, 0.48)]):
        g[5][i * 24] = n(CHOIR, note, vel, 20)
    # Bass enters softly
    g[3][0]  = n(BASS, E2, 0.40, 16)
    g[3][24] = n(BASS, C3, 0.38, 16)
    g[3][48] = n(BASS, A2, 0.36, 16)
    g[3][72] = n(BASS, E2, 0.40, 16)
    # Harp arpeggios: each chord broken
    harp_arp_up(g, 4, E4, G4, B4, E5, vel=0.45)
    harp_arp_up(g, 28, C4, E4, G4, C5, vel=0.42)
    harp_arp_down(g, 52, A4, E4, C4, A3, vel=0.40)
    harp_arp_up(g, 76, E4, G4, B4, E5, vel=0.45)
    # Flute: journey motif — E→G→A→B, lyrical and slow
    flute_phrase(g, 8, [(E4, 4), (G4, 4), (A4, 4), (B4, 6)], vel=0.55)
    # Second phrase: descending answer — B→A→G→E
    flute_phrase(g, 32, [(B4, 3), (A4, 3), (G4, 4), (E4, 6)], vel=0.52)
    # Third: longing upward reach — E→G→B→D5, pause
    flute_phrase(g, 56, [(E4, 3), (G4, 3), (B4, 4), (D5, 8)], vel=0.58)
    # Gentle drums enter midway
    add_gentle_drums(g, 48, 3, vel_k=0.28, vel_h=0.18)
    # Bell accents
    g[7][20] = n(BELL, G5, 0.28, 3)
    g[7][68] = n(BELL, B5, 0.25, 4)
    return Pattern(name="cosmic_grief", bpm=BPM, steps_per_beat=SPB,
                   num_steps=96, num_channels=NCH, grid=g)


def p_gifts_journey() -> Pattern:
    """Act II — The Four Gifts. Warmer, more hopeful. Drums build. 160 steps (10 bars)."""
    g = grid(160)
    # ── Lantern section (0-39) ──
    g[5][0]  = n(CHOIR, G3, 0.50, 16)
    g[5][16] = n(CHOIR, E3, 0.48, 16)
    g[3][0]  = n(BASS, G2, 0.45, 12)
    g[3][16] = n(BASS, E2, 0.42, 12)
    # Harp: G major arp (warm, hopeful)
    harp_arp_up(g, 0, G4, B4, D5, G5, vel=0.55)
    harp_arp_down(g, 16, E5, B4, G4, E4, vel=0.50)
    # Flute: lantern discovery — ascending wonder
    flute_phrase(g, 4, [(G4, 3), (A4, 2), (B4, 3), (D5, 4), (E5, 4)], vel=0.62)
    flute_phrase(g, 24, [(D5, 3), (B4, 3), (G4, 4), (A4, 4)], vel=0.58)
    # Bell: lantern ignites
    g[7][8] = n(BELL, G5, 0.40, 4)
    add_gentle_drums(g, 0, 2, vel_k=0.32, vel_h=0.20)

    # ── Mirror section (40-79) ──
    g[5][40] = n(CHOIR, C4, 0.48, 16)
    g[5][56] = n(CHOIR, A3, 0.45, 16)
    g[3][40] = n(BASS, C3, 0.42, 12)
    g[3][56] = n(BASS, A2, 0.40, 12)
    harp_arp_up(g, 40, C4, E4, G4, C5, vel=0.52)
    harp_arp_up(g, 56, A3, C4, E4, A4, vel=0.50)
    # Flute: mirror — two truths (call and answer)
    flute_phrase(g, 44, [(C5, 3), (E5, 3), (D5, 4)], vel=0.60)
    flute_phrase(g, 60, [(E4, 3), (G4, 3), (A4, 4), (B4, 4)], vel=0.58)
    g[7][48] = n(BELL, E5, 0.35, 3)
    g[7][52] = n(BELL, C5, 0.32, 3)  # two bells = two truths
    add_gentle_drums(g, 40, 2, vel_k=0.35, vel_h=0.22)

    # ── Hammer section (80-119) — building energy ──
    g[5][80]  = n(CHOIR, D4, 0.52, 16)
    g[5][96]  = n(CHOIR, G3, 0.50, 16)
    g[3][80]  = n(BASS, D3, 0.45, 12)
    g[3][96]  = n(BASS, G2, 0.48, 12)
    harp_arp_up(g, 80, D4, Fs4, A4, D5, vel=0.55)
    harp_arp_up(g, 96, G4, B4, D5, G5, vel=0.58)
    # Flute: building — stronger, more rhythmic
    flute_phrase(g, 84, [(D5, 2), (E5, 2), (Fs5, 3), (G5, 3), (Fs5, 2), (E5, 2)], vel=0.65)
    flute_phrase(g, 104, [(B4, 3), (D5, 3), (G5, 4), (Fs5, 4)], vel=0.62)
    g[7][88] = n(BELL, D5, 0.38, 3)
    add_full_drums(g, 80, 2, vel_k=0.40, vel_s=0.32, vel_h=0.25)

    # ── Chain section (120-159) — weight, gravity ──
    g[5][120] = n(CHOIR, E3, 0.55, 16)
    g[5][136] = n(CHOIR, B3, 0.50, 20)
    g[3][120] = n(BASS, E2, 0.50, 16)
    g[3][136] = n(BASS, B2, 0.48, 16)
    harp_arp_down(g, 120, B4, G4, E4, B3, vel=0.50)
    harp_arp_down(g, 136, E5, B4, G4, E4, vel=0.48)
    # Flute: heavy chain motif — slower, weighted
    flute_phrase(g, 124, [(E4, 4), (G4, 4), (B4, 6), (A4, 4)], vel=0.60)
    flute_phrase(g, 144, [(G4, 4), (E4, 6), (D4, 4)], vel=0.55)
    g[7][128] = n(BELL, E4, 0.30, 6)  # deep bell = weight
    add_gentle_drums(g, 120, 2, vel_k=0.38, vel_h=0.22, vel_r=0.28)

    return Pattern(name="gifts_journey", bpm=BPM, steps_per_beat=SPB,
                   num_steps=160, num_channels=NCH, grid=g)


def p_labyrinth() -> Pattern:
    """Act III — The maze. Darker, more uncertain. 128 steps (8 bars)."""
    g = grid(128)
    # Choir: dark minor chords — Em → Dm → Em → Bm
    for i, (note, vel) in enumerate([(E3, 0.50), (D3, 0.48), (E3, 0.48), (B2, 0.52)]):
        g[5][i * 32] = n(CHOIR, note, vel, 28)
    # Bass: steady dark pulse
    g[3][0]  = n(BASS, E2, 0.48, 16)
    g[3][16] = n(BASS, E2, 0.45, 12)
    g[3][32] = n(BASS, D3, 0.44, 16)
    g[3][48] = n(BASS, D3, 0.42, 12)
    g[3][64] = n(BASS, E2, 0.46, 16)
    g[3][80] = n(BASS, E2, 0.44, 12)
    g[3][96] = n(BASS, B2, 0.50, 16)
    g[3][112] = n(BASS, B2, 0.48, 12)
    # Harp: nervous, shorter arpeggios — searching
    for bar in range(8):
        base = bar * 16
        if bar % 2 == 0:
            harp_arp_up(g, base + 2, E4, G4, B4, E5, vel=0.42, dur=2)
        else:
            harp_arp_down(g, base + 2, D5, A4, Fs4, D4, vel=0.40, dur=2)
    # Flute: wandering, searching phrases
    flute_phrase(g, 8, [(E4, 3), (Fs4, 2), (G4, 3), (A4, 3), (G4, 3)], vel=0.55)
    flute_phrase(g, 40, [(D4, 3), (E4, 3), (Fs4, 4), (G4, 4)], vel=0.52)
    # Thread found — ascending hope
    flute_phrase(g, 72, [(B4, 4), (D5, 4), (E5, 6)], vel=0.60)
    # Mapping walls — methodical
    flute_phrase(g, 96, [(E4, 2), (G4, 2), (A4, 2), (B4, 2), (A4, 2), (G4, 2)], vel=0.50)
    # This was enough — settling
    flute_phrase(g, 112, [(E4, 4), (G4, 4), (B4, 8)], vel=0.55)
    # Drums: very sparse (lost in maze)
    add_gentle_drums(g, 0, 4, vel_k=0.25, vel_h=0.16, vel_r=0.20)
    # No drums bars 5-6 (stillness while mapping)
    add_gentle_drums(g, 96, 2, vel_k=0.28, vel_h=0.18)
    g[7][76] = n(BELL, B5, 0.32, 4)  # thread discovery bell
    return Pattern(name="labyrinth", bpm=BPM, steps_per_beat=SPB,
                   num_steps=128, num_channels=NCH, grid=g)


def p_thread_hope() -> Pattern:
    """Brief peaceful interlude — 'this was enough.' 80 steps (5 bars)."""
    g = grid(80)
    g[5][0]  = n(CHOIR, G3, 0.48, 16)
    g[5][16] = n(CHOIR, C4, 0.45, 16)
    g[5][32] = n(CHOIR, A3, 0.43, 16)
    g[5][48] = n(CHOIR, E3, 0.48, 20)
    g[3][0]  = n(BASS, G2, 0.42, 16)
    g[3][32] = n(BASS, A2, 0.40, 16)
    g[3][48] = n(BASS, E2, 0.44, 20)
    # Harp: peaceful, flowing
    harp_arp_up(g, 0, G4, B4, D5, G5, vel=0.50, dur=3)
    harp_arp_up(g, 16, C4, E4, G4, C5, vel=0.48, dur=3)
    harp_arp_down(g, 32, A4, E4, C4, A3, vel=0.45, dur=3)
    harp_arp_up(g, 48, E4, G4, B4, E5, vel=0.50, dur=3)
    # Flute: serene, at peace
    flute_phrase(g, 4, [(G4, 4), (B4, 4), (D5, 4), (E5, 6)], vel=0.58)
    flute_phrase(g, 36, [(A4, 3), (G4, 3), (E4, 4), (D4, 4)], vel=0.52)
    flute_phrase(g, 56, [(E4, 4), (G4, 6), (B4, 8)], vel=0.55)
    # No drums — pure peace
    g[7][8]  = n(BELL, D5, 0.28, 4)
    g[7][40] = n(BELL, G5, 0.25, 5)
    return Pattern(name="thread_hope", bpm=BPM, steps_per_beat=SPB,
                   num_steps=80, num_channels=NCH, grid=g)


def p_unraveling() -> Pattern:
    """Act IV — 'But the universe is patient.' Dark turn. Full drums. 128 steps."""
    g = grid(128)
    # Choir: dark, heavy — Em → Cm → Em → Bm
    g[5][0]  = n(CHOIR, E3, 0.58, 28)
    g[5][32] = n(CHOIR, C3, 0.55, 28)
    g[5][64] = n(CHOIR, E3, 0.60, 28)
    g[5][96] = n(CHOIR, B2, 0.62, 28)
    # Bass: heavy, driving
    for bar in range(8):
        base = bar * 16
        root = [E2, E2, C3, C3, E2, E2, B2, B2][bar]
        g[3][base] = n(BASS, root, 0.55, 8)
        g[3][base + 8] = n(BASS, root, 0.50, 6)
    # Harp: agitated, descending — things falling apart
    for bar in range(8):
        base = bar * 16
        if bar < 4:
            harp_arp_down(g, base + 2, E5, B4, G4, E4, vel=0.48)
        else:
            harp_arp_down(g, base + 2, B4, G4, E4, B3, vel=0.50)
    # Flute: anguished, dramatic phrases
    # "universe is patient" — slow, ominous
    flute_phrase(g, 4, [(E4, 4), (D4, 4), (C4, 6)], vel=0.60)
    # "walls shifted" — jagged
    flute_phrase(g, 24, [(G4, 2), (Fs4, 2), (E4, 2), (D4, 2), (C4, 4)], vel=0.58)
    # "thread frays" — climbing, desperate
    flute_phrase(g, 48, [(E4, 3), (G4, 3), (A4, 3), (B4, 3), (C5, 4)], vel=0.65)
    # "things crumbled" — falling
    flute_phrase(g, 68, [(B4, 3), (A4, 3), (G4, 3), (E4, 3), (D4, 4)], vel=0.62)
    # "lantern dimmed" — fading
    flute_phrase(g, 88, [(E4, 4), (D4, 6), (C4, 6)], vel=0.55)
    # "chain held" — low, anchor
    flute_phrase(g, 108, [(E4, 6), (G4, 4), (A4, 4), (B4, 6)], vel=0.60)
    # Full drums — the storm
    add_full_drums(g, 0, 8, vel_k=0.50, vel_s=0.42, vel_h=0.28)
    g[7][16] = n(BELL, E4, 0.30, 3)  # dark bell
    g[7][72] = n(BELL, C5, 0.35, 3)  # crumbling
    return Pattern(name="unraveling", bpm=BPM, steps_per_beat=SPB,
                   num_steps=128, num_channels=NCH, grid=g)


def p_stood_up() -> Pattern:
    """'He stood up anyway.' — Triumph moment. Major turn. 64 steps."""
    g = grid(64)
    # Choir: MAJOR chord — G major → D major → Em → G
    g[5][0]  = n(CHOIR, G3, 0.62, 14)
    g[5][16] = n(CHOIR, D4, 0.60, 14)
    g[5][32] = n(CHOIR, E3, 0.58, 14)
    g[5][48] = n(CHOIR, G3, 0.65, 14)
    g[3][0]  = n(BASS, G2, 0.55, 12)
    g[3][16] = n(BASS, D3, 0.52, 12)
    g[3][32] = n(BASS, E2, 0.50, 12)
    g[3][48] = n(BASS, G2, 0.55, 12)
    # Harp: triumphant ascending arpeggios
    harp_arp_up(g, 0, G4, B4, D5, G5, vel=0.62, dur=2)
    harp_arp_up(g, 16, D4, Fs4, A4, D5, vel=0.60, dur=2)
    harp_arp_up(g, 32, E4, G4, B4, E5, vel=0.58, dur=2)
    harp_arp_up(g, 48, G4, B4, D5, G5, vel=0.65, dur=2)
    # Flute: JOURNEY MOTIF — triumphant return, higher, fuller
    flute_phrase(g, 4, [(E5, 3), (G5, 3), (A5, 4), (B5, 6)], vel=0.72)
    # Answer: ringing, sustained
    flute_phrase(g, 24, [(B5, 4), (G5, 4), (E5, 4), (G5, 6)], vel=0.68)
    # Resolving
    flute_phrase(g, 48, [(G5, 4), (Fs5, 4), (E5, 8)], vel=0.65)
    # Full drums with energy
    add_full_drums(g, 0, 4, vel_k=0.52, vel_s=0.45, vel_h=0.30)
    # Bell fanfare
    g[7][0]  = n(BELL, G5, 0.45, 4)
    g[7][16] = n(BELL, D5, 0.40, 4)
    g[7][32] = n(BELL, E5, 0.42, 4)
    g[7][48] = n(BELL, G5, 0.50, 6)
    return Pattern(name="stood_up", bpm=BPM, steps_per_beat=SPB,
                   num_steps=64, num_channels=NCH, grid=g)


def p_universe_returns() -> Pattern:
    """Act V — 'The universe does not forget.' Cosmic warmth. 96 steps."""
    g = grid(96)
    # Choir: warm, celestial — G → C → Em → G
    g[5][0]  = n(CHOIR, G3, 0.58, 20)
    g[5][24] = n(CHOIR, C4, 0.55, 20)
    g[5][48] = n(CHOIR, E3, 0.52, 20)
    g[5][72] = n(CHOIR, G3, 0.58, 20)
    g[3][0]  = n(BASS, G2, 0.48, 16)
    g[3][24] = n(BASS, C3, 0.45, 16)
    g[3][48] = n(BASS, E2, 0.46, 16)
    g[3][72] = n(BASS, G2, 0.48, 16)
    # Harp: flowing, celestial
    harp_arp_up(g, 0, G4, B4, D5, G5, vel=0.55, dur=3)
    harp_arp_up(g, 24, C4, E4, G4, C5, vel=0.52, dur=3)
    harp_arp_down(g, 48, E5, B4, G4, E4, vel=0.50, dur=3)
    harp_arp_up(g, 72, G4, B4, D5, G5, vel=0.55, dur=3)
    # Flute: reuniting melody — gentle, returning home
    flute_phrase(g, 4, [(G4, 4), (B4, 4), (D5, 4), (G5, 6)], vel=0.60)
    flute_phrase(g, 28, [(E5, 4), (D5, 3), (C5, 3), (B4, 4)], vel=0.55)
    flute_phrase(g, 52, [(E4, 4), (G4, 4), (A4, 4), (B4, 6)], vel=0.58)
    flute_phrase(g, 80, [(D5, 4), (B4, 4), (G4, 8)], vel=0.52)
    # Gentle drums return
    add_gentle_drums(g, 0, 6, vel_k=0.32, vel_h=0.18, vel_r=0.22)
    g[7][8]  = n(BELL, G5, 0.35, 5)
    g[7][56] = n(BELL, E5, 0.30, 5)
    return Pattern(name="universe_returns", bpm=BPM, steps_per_beat=SPB,
                   num_steps=96, num_channels=NCH, grid=g)


def p_walk_home() -> Pattern:
    """'The boy kept walking.' Warm, resolving. 96 steps."""
    g = grid(96)
    # Choir: resolving — Em → G → C → G
    g[5][0]  = n(CHOIR, E3, 0.52, 20)
    g[5][24] = n(CHOIR, G3, 0.55, 20)
    g[5][48] = n(CHOIR, C4, 0.50, 20)
    g[5][72] = n(CHOIR, G3, 0.52, 20)
    g[3][0]  = n(BASS, E2, 0.45, 16)
    g[3][24] = n(BASS, G2, 0.48, 16)
    g[3][48] = n(BASS, C3, 0.42, 16)
    g[3][72] = n(BASS, G2, 0.45, 16)
    # Harp: final Moonlight-style arpeggios — gentle, accepting
    harp_arp_up(g, 0, E4, G4, B4, E5, vel=0.48, dur=3)
    harp_arp_up(g, 24, G4, B4, D5, G5, vel=0.52, dur=3)
    harp_arp_down(g, 48, C5, G4, E4, C4, vel=0.45, dur=3)
    harp_arp_up(g, 72, G4, B4, D5, G5, vel=0.48, dur=3)
    # Flute: journey motif — one last time, tender
    flute_phrase(g, 4, [(E4, 4), (G4, 4), (A4, 4), (B4, 6)], vel=0.55)
    flute_phrase(g, 28, [(D5, 4), (B4, 4), (G4, 6)], vel=0.50)
    flute_phrase(g, 52, [(C5, 4), (B4, 3), (A4, 3), (G4, 4)], vel=0.48)
    # "There is no ending" — the motif ascending one final time
    flute_phrase(g, 72, [(E4, 4), (G4, 4), (B4, 4), (E5, 8)], vel=0.55)
    add_gentle_drums(g, 0, 4, vel_k=0.28, vel_h=0.16)
    # No drums last 2 bars — just music breathing
    g[7][8]  = n(BELL, B5, 0.28, 5)
    g[7][76] = n(BELL, E5, 0.30, 6)
    return Pattern(name="walk_home", bpm=BPM, steps_per_beat=SPB,
                   num_steps=96, num_channels=NCH, grid=g)


def p_fadeout() -> Pattern:
    """Credits music — warmth dissolving. Harp + choir only. 128 steps.
    
    User loves this section — keep it beautiful and long.
    """
    g = grid(128)
    # Choir: long, fading — G → Em → C → G → Em → ...
    chords = [G3, E3, C4, G3, E3, C4, G3, E3]
    vels   = [0.48, 0.44, 0.40, 0.36, 0.32, 0.28, 0.24, 0.18]
    for i in range(8):
        g[5][i * 16] = n(CHOIR, chords[i], vels[i], 14)
    # Bass: dying away
    for i in range(4):
        root = [G2, E2, C3, G2][i]
        g[3][i * 32] = n(BASS, root, 0.36 - i * 0.06, 16)
    # Harp: increasingly sparse — like stars winking out
    harp_arp_up(g, 0, G4, B4, D5, G5, vel=0.42, dur=3)
    harp_arp_down(g, 24, E5, B4, G4, E4, vel=0.38, dur=3)
    harp_arp_up(g, 48, C4, E4, G4, C5, vel=0.32, dur=4)
    # Just two lonely notes
    g[4][80] = n(HARP, G4, 0.25, 6)
    g[4][96] = n(HARP, E4, 0.20, 6)
    # One last harp note
    g[4][112] = n(HARP, B4, 0.15, 8)
    # Flute: one final sustained note — the last breath
    g[6][16] = n(FLUTE, E5, 0.40, 8)
    g[6][48] = n(FLUTE, B4, 0.32, 8)
    g[6][80] = n(FLUTE, G4, 0.25, 10)
    # Bells: fading
    g[7][4]  = n(BELL, G5, 0.25, 5)
    g[7][36] = n(BELL, E5, 0.20, 5)
    g[7][72] = n(BELL, B4, 0.15, 6)
    g[7][108] = n(BELL, E5, 0.10, 8)  # the last star
    return Pattern(name="fadeout", bpm=BPM, steps_per_beat=SPB,
                   num_steps=128, num_channels=NCH, grid=g)


def p_silence() -> Pattern:
    """32 steps of silence — breathing room before tail."""
    g = grid(32)
    # Just the faintest choir drone
    g[5][0] = n(CHOIR, E3, 0.10, 16)
    return Pattern(name="silence", bpm=BPM, steps_per_beat=SPB,
                   num_steps=32, num_channels=NCH, grid=g)


def p_tail() -> Pattern:
    """32-step tail — one final bell, then true silence."""
    g = grid(32)
    g[7][0] = n(BELL, E5, 0.12, 8)
    g[5][0] = n(CHOIR, E3, 0.06, 12)
    return Pattern(name="tail", bpm=BPM, steps_per_beat=SPB,
                   num_steps=32, num_channels=NCH, grid=g)
...
def build_song() -> Song:
    patterns = [
        p_starfield(),         # 0  — 64 steps
        p_cosmic_grief(),      # 1  — 96 steps
        p_gifts_journey(),     # 2  — 160 steps
        p_labyrinth(),         # 3  — 128 steps
        p_thread_hope(),       # 4  — 80 steps
        p_unraveling(),        # 5  — 128 steps
        p_stood_up(),          # 6  — 64 steps
        p_universe_returns(),  # 7  — 96 steps
        p_walk_home(),         # 8  — 96 steps
        p_fadeout(),           # 9  — 128 steps
        p_silence(),           # 10 — 32 steps
        p_tail(),              # 11 — 32 steps
    ]
    # Total: 64+96+160+128+80+128+64+96+96+128+32+32 = 1104 steps
    sequence = list(range(len(patterns)))

    channel_effects = {
        0: {"reverb": 0.08},                                              # kick
        1: {"reverb": 0.15},                                              # snare/ride
        2: {"delay": 0.10, "delay_feedback": 0.15, "reverb": 0.06},      # hats
        3: {"reverb": 0.10},                                              # bass
        4: {"reverb": 0.38, "delay": 0.22, "delay_feedback": 0.28},      # harp
        5: {"reverb": 0.48},                                              # choir
        6: {"reverb": 0.35, "delay": 0.18, "delay_feedback": 0.22},      # flute
        7: {"reverb": 0.42, "delay": 0.25, "delay_feedback": 0.30},      # bell
    }
    panning = {
        0: 0.0,     # kick center
        1: 0.05,    # snare slight right
        2: 0.25,    # hats right
        3: -0.05,   # bass slight left
        4: -0.22,   # harp left
        5: -0.10,   # choir slight left
        6: 0.12,    # flute slight right
        7: 0.28,    # bell right
    }
...
if __name__ == "__main__":
    ...
    total_steps = sum(song.patterns[i].num_steps for i in song.sequence)
    secs = total_steps / (BPM / 60 * SPB)
    print(f"Melody v5: {len(song.patterns)} patterns, {total_steps} steps, "
          f"{secs:.1f}s at {BPM} BPM")

    audio = render_song(song, panning=song.panning,
                        channel_effects=song.channel_effects,
                        master_reverb=song.master_reverb,
                        master_delay=getattr(song, 'master_delay', 0.0))
